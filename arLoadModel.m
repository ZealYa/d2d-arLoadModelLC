...
if(strcmp(C{1},'REACTIONS') || strcmp(C{1},'REACTIONS-AMOUNTBASED'))
    ...
    while(~strcmp(str{1},'INVARIANTS') && ~strcmp(str{1},'DERIVED'))
        ...
        if delayCh(chainID).ctr ~= 0 && delayCh(chainID).ctr < delayCh(chainID).length
            line = sprintf('%s_%02d -> %s_%02d CUSTOM "%s_%s*%s_%02d"',...
                delayCh(chainID).stname, delayCh(chainID).ctr, delayCh(chainID).stname, ...
                delayCh(chainID).ctr+1, chainpar, delayCh(chainID).stname, delayCh(chainID).stname, delayCh(chainID).ctr);
            remainder = line;
            delayCh(chainID).ctr = delayCh(chainID).ctr + 1;
        end     
